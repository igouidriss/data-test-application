  ...
  return (
    <div>
      <h2 id="room-heading" data-cy="RoomHeading">
        <Translate contentKey="dataTestApplicationApp.room.home.title">Rooms</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="dataTestApplicationApp.room.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/room/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="dataTestApplicationApp.room.home.createLabel">Create new Room</Translate>
          </Link>
        </div>
      </h2>
        ...
        {roomList && roomList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="dataTestApplicationApp.room.id">ID</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('seatsNumber')}>
                  <Translate contentKey="dataTestApplicationApp.room.seatsNumber">Seats Number</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('seatsNumber')} />
                </th>
                <th className="hand" onClick={sort('screenHeight')}>
                  <Translate contentKey="dataTestApplicationApp.room.screenHeight">Screen Height</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('screenHeight')} />
                </th>
                <th className="hand" onClick={sort('screenWidth')}>
                  <Translate contentKey="dataTestApplicationApp.room.screenWidth">Screen Width</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('screenWidth')} />
                </th>
                ...
                <th>
                  <Translate contentKey="dataTestApplicationApp.room.cinema">Cinema</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {roomList.map((room, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{room.screenHeight}</td>
                  <td>{room.screenWidth}</td>
                  <td>{room.distance}</td>
                  <td>{room.cinema ? <Link to={`/cinema/${room.cinema.id}`}>{room.cinema.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="dataTestApplicationApp.room.home.notFound">No Rooms found</Translate>
            </div>
          )
        )}
      </div>
